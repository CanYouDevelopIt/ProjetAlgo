package algo.builder;

public interface Images {
	public static final String fromage = "Images/fromage.png";
	public static final String buisson = "Images/buisson.png";
	public static final String depart = "Images/depart.png";
	public static final String espace = "Images/espace.png";
	public static final String mur = "Images/mur.png";
	public static final String souris = "Images/souris.png";

}